...
def load_files_into_db(ds, **kwargs):
    ...
    with engine.connect() as conn:
        conn.execute("drop table if exists dx_file cascade ")

        df_npi_small = pd.read_csv(os.path.join(GE_TUTORIAL_ROOT_PATH, "data", "diagnosis.csv"), sep='\t')
        ...
        df_npi_small.to_sql("dx_file", engine,
                            schema=None,
                            if_exists='replace',
                            index=False,
                            index_label=None,
...
def validate_source_data(ds, **kwargs):
    """
        A function to validate data from a CSV file .
    """

    context = ge.data_context.DataContext(great_expectations_context_path)

    batch_kwargs_file = {"path": os.path.join(GE_TUTORIAL_ROOT_PATH, "data", "diagnosis.csv"),
                         'datasource': 'input_files',
                         "reader_options": {"sep": "\t"}}

    batch_file = context.get_batch(batch_kwargs_file, 'dx_file.critical')
...
def validate_source_data_load(ds, **kwargs):
    ...
    expectation_suite_name_file = "dx_file.critical"
    batch_kwargs_file = {"path": os.path.join(GE_TUTORIAL_ROOT_PATH, "data", "diagnosis.csv"),
                         'datasource': 'input_files',
                         "reader_options": {"sep": "\t"}}
    ...
    expectation_suite_name_db = "dx_db_table.critical"
    ...
    batch_kwargs_db = {'table': "dx_file", 'datasource': datasource_name_file_db}
